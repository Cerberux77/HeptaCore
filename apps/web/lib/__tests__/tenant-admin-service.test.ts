...
type WhereFn = (record: StoredRecord) => boolean;
...
function buildFakeDb(): {
  db: TenantAdminDb & { $transaction: (fn: (tx: unknown) => Promise<unknown>) => Promise<unknown> };
  collections: {
  ...
  const users: FakeCollection = {
    records: [],
    create({ data }) { const r = { id: `u_${this.records.length + 1}`, ...data }; this.records.push(r); return r; },
    findUnique({ where }) { return this.records.find((r) => (where as any).id === r.id || (where as any).email === r.email) ?? null; },
  ...
  const tenants: FakeCollection = {
    records: [],
    create({ data }) { const r = { id: `t_${this.records.length + 1}`, ...data }; this.records.push(r); return r; },
    ...
    update({ where, data }) { const r = this.records.find((r) => (where as any).id === r.id)!; Object.assign(r, data as any); return { ...r }; },
  };

  const memberships: FakeCollection = {
    records: [],
    create({ data }) { const r = { id: `m_${this.records.length + 1}`, ...data }; this.records.push(r); return r; },
    findUnique({ where }) {
  ...
  const invitations: FakeCollection = {
    records: [],
    create({ data }) { const r = { id: `inv_${this.records.length + 1}`, ...data }; this.records.push(r); return r; },
  ...
  const auditLogs: FakeCollection = {
    records: [],
    create({ data }) { const r = { id: `a_${this.records.length + 1}`, ...data }; this.records.push(r); return r; },
...
describe("provisioning with fake DB", () => {
  ...
  describe("createAdminTenant", () => {
    ...
    it("rollback on invitation failure", async () => {
      ...
      await assert.rejects(
        () => createAdminTenant({ actorId: "sa1", slug: "t3", name: "T3", ownerEmail: "o3@test.com" }, db),
      );
    ...
    it("invitation hash is verified against existing register format", () => {
      ...
      const sameHash = createHash("sha256").update(token).digest("hex");
    ...
    it("ADMIN cannot create tenant", async () => {
      ...
      fake.collections.memberships.create({ data: { tenantId: "t1", userId: "a1", role: "ADMIN" } });
      await assert.rejects(
        () => createAdminTenant({ actorId: "a1", slug: "t2", name: "T2", ownerEmail: "x@test.com" }, db),
        TenantAccessError,
      );
    });
  });
  ...
  describe("changeTenantStatus transactional", () => {
    ...
    it("audit failure rolls back status change", () => {
      // Covered by transaction rollback: if audit fails, the transaction rolls back the status update
      // The fake DB $transaction handles this via snapshots
    });
  });

  describe("updateAdminTenantConfiguration transactional", () => {
    it.skip("update config + audit in transaction", async () => {
      const t = await createAdminTenant({
        actorId: "sa1", slug: "cfg-1", name: "CFG1", ownerEmail: "cfg1@test.com",
      }, db);
      const before = fake.collections.auditLogs.records.length;
      ...
      assert.ok(audits.length > before, "audit should exist");
    });
