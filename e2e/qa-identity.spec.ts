...
if (!PASSWORD) {
  test.skip("HEPTACORE_QA_E2E_PASSWORD not set");
}

const TENANT_SLUG = "qa-e2e-active";
...
test.describe("QA Identity & Session", () => {
  ...
  test("2 - OWNER enters /tenant/qa-e2e-active and sees Propietario role", async ({ page }) => {
    ...
    await page.fill('input[name="password"]', PASSWORD);
    await page.click('button[type="submit"]');
    await page.waitForURL(/\/tenant\/qa-e2e-active/, { timeout: 15000 });
  ...
  test("4 - VIEWER enters, can read tenant but has limited permissions", async ({ page }) => {
    await page.goto("/login");
